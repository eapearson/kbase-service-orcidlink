...
class ServiceUtils:
    ...
    def obj_info_to_object(obj_info):
        ...
        [type_module, type_name, type_major_version, type_minor_version] = re.split(
            r"-|\.", type
        )
        ...
        return {
            ...
            "type_info": {
                "module": type_module,
                "name": type_name,
                "version": {"major": type_major_version, "minor": type_minor_version},
            },
    ...
    def parse_app_key(key):
        parts = list(filter(lambda part: len(part) > 0, key.split("/")))
        if len(parts) == 1:
            return {"name": parts[0], "shortRef": parts[0], "ref": parts[0]}
        elif len(parts) == 3:
            return {
                "module": parts[0],
                "name": parts[1],
