...
def test_transform_work_summary():
    ...
    with pytest.raises(
        ...
        orcid_data = orcid_api.WorkSummary.parse_obj(test_work_summary_no_doi)
    ...
    with pytest.raises(
        ...
        orcid_data = orcid_api.WorkSummary.parse_obj(test_work_summary_multi_doi)
        to_service.transform_work_summary(orcid_data)


def test_transform_work():
    test_work = orcid_api.Work.parse_obj(
        load_test_data("orcid", "work_1526002")["bulk"][0]["work"]
    )
    test_profile = orcid_api.ORCIDProfile.parse_obj(load_test_data("orcid", "profile"))
    test_work_transformed = model.Work.parse_obj(
...
def test_transform_external_id():
    ...
    external_id_orcid = orcid_api.ExternalId.parse_obj(external_id_orcid_data)
...
def test_transform_orcid_profile_no_credit_name():
    ...
    orcid_profile = orcid_api.ORCIDProfile.parse_obj(raw_test_profile)
