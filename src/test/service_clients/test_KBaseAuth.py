...
async def test_KBaseAuth_get_token_info(fake_fs):
...
async def test_KBaseAuth_get_token_info_other_error(fake_fs):
    with mock_services() as url:
        client = KBaseAuth(url=url, cache_max_size=3, cache_lifetime=3)
        assert client is not None
        client.cache.clear()
...
async def test_KBaseAuth_get_token_info_internal_error(fake_fs):
    with mock_services() as url:
        client = KBaseAuth(url=url, cache_max_size=3, cache_lifetime=3)
        assert client is not None
        client.cache.clear()
...
async def test_KBaseAuth_get_token_info_param_errors(fake_fs):
    client = KBaseAuth(
...
async def test_KBaseAuth_get_username(fake_fs):
    with mock_services() as url:
        client = KBaseAuth(url=url, cache_max_size=3, cache_lifetime=3)
        assert client is not None
        client.cache.clear()

        username = await client.get_username("foo")
        assert username == "foo"
